...
import { TrendingUp, Clock, Target, Award, CheckCircle2, Plus, Trash2, Edit2, CheckSquare, Square, ListTodo } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const daysActive = useMemo(() => {
    ...
    const sortedDates = [...new Set(sessions.map(s => new Date(s.endTime).toDateString()))];
  ...
  return (
    <div className="space-y-6 pb-20 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{totalHours} hrs</h3>
            </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center space-x-4">
            ...
            <div>
                ...
                <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{totalSessions}</h3>
            </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center space-x-4">
            ...
            <div>
                <p className="text-sm text-slate-500 dark:text-slate-400">Días Activos</p>
                <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">{daysActive}</h3>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
           <div className="flex justify-between items-center mb-4">
              ...
              {!isEditingTarget ? (
                 <button onClick={() => setIsEditingTarget(true)} className="text-slate-400 hover:text-primary-500 transition-colors">
                   <Edit2 className="w-4 h-4" />
                 </button>
              ) : (
                <button onClick={saveTarget} className="text-green-500 font-bold text-xs bg-green-100 dark:bg-green-900 px-2 py-1 rounded">
                  Guardar
                </button>
              )}
           </div>
           ...
           <div className="flex flex-col items-center">
             {isEditingTarget ? (
               <div className="flex items-center space-x-2 mb-4">
                ...
                <div className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-2">
                  {weeklyProgress.hours} <span className="text-base font-normal text-slate-500">/ {weeklyTarget} hrs</span>
                </div>
             )}
             ...
             <p className="text-xs text-slate-500 text-center">
               {weeklyProgress.percentage >= 100 
                  ...
                  : `Te faltan ${(weeklyTarget - weeklyProgress.hours).toFixed(1)} horas para tu meta.`
               }
             </p>
           </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto max-h-60 space-y-2 mb-4 pr-1 custom-scrollbar">
            {objectives.length === 0 ? (
               <p className="text-slate-400 text-sm italic text-center py-4">Agrega tareas para mantenerte organizado.</p>
            ) : (
              objectives.map(obj => (
                <div key={obj.id} className="group flex items-center justify-between p-2 hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-lg transition-colors">
                  <div className="flex items-center space-x-3 overflow-hidden">
                    <button onClick={() => onToggleObjective(obj.id)} className="flex-shrink-0 text-slate-400 hover:text-pink-500 transition-colors">
                      {obj.isCompleted ? <CheckSquare className="w-5 h-5 text-pink-500" /> : <Square className="w-5 h-5" />}
                    </button>
                    <span className={`text-sm truncate ${obj.isCompleted ? 'line-through text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>
                      {obj.text}
                    </span>
                  </div>
                  <button onClick={() => onDeleteObjective(obj.id)} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-red-500 transition-opacity">
          ...
          <form onSubmit={handleAddObjectiveSubmit} className="relative mt-auto">
             ...
             />
             <button type="submit" className="absolute right-2 top-1/2 transform -translate-y-1/2 text-primary-600 dark:text-primary-400 hover:scale-110 transition-transform">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Weekly Activity */}
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-lg font-semibold mb-4 flex items-center"><TrendingUp className="w-5 h-5 mr-2 text-primary-500"/> Actividad Semanal (min)</h3>
            <div className="h-64">
        ...
        <div className="bg-white dark:bg-slate-900 p-6 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
            <h3 className="text-lg font-semibold mb-4">Por Asignatura</h3>
            <div className="h-64">
                <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                        <Pie
